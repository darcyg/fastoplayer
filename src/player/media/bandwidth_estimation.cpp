...
DesireBytesPerSec::DesireBytesPerSec(bandwidth_t min, bandwidth_t max)
    : min(min), max(max) {}
...
DesireBytesPerSec &DesireBytesPerSec::
operator+=(const DesireBytesPerSec &other) {
  min += other.min;
  max += other.max;
  return *this;
}

DesireBytesPerSec CalculateDesireAudioBandwidthBytesPerSec(int rate,
                                                           int channels) {
  int min_bitrate_kbps = rate * channels / 1000;
  return DesireBytesPerSec(KBITS_TO_BYTES(min_bitrate_kbps),
                           KBITS_TO_BYTES(min_bitrate_kbps * 2));
}
...
DesireBytesPerSec CalculateDesireH264BandwidthBytesPerSec(int width, int height,
                                                          double framerate,
                                                          int profile) {
  ...
  const int stabled_profile =
      stable_value_in_range(profile, PROFILE_H264_BASELINE, PROFILE_H264_HIGH);
  ...
  WARNING_LOG() << "Size: " << width << "x" << height
                << ", framerate: " << framerate << ", profile: " << profile;
  return DesireBytesPerSec(KBITS_TO_BYTES(0), KBITS_TO_BYTES(0));
}

DesireBytesPerSec CalculateDesireMPEGBandwidthBytesPerSec(int width,
                                                          int height) {
