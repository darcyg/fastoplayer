...
static int dxva2_alloc(AVCodecContext* s) {
  ...
  int ret = av_hwdevice_ctx_create(&ctx->hw_device_ctx, AV_HWDEVICE_TYPE_DXVA2, ist->hwaccel_device, NULL, 0);
  if (ret < 0) {
    dxva2_uninit(s);
  ...
  void* pv = NULL;
  hr = IDirect3DDeviceManager9_GetVideoService(device_hwctx->devmgr, device_handle, IID_IDirectXVideoDecoderService,
                                               &pv);
...
static int dxva2_get_decoder_configuration(AVCodecContext* s,
  ...
  HRESULT hr = IDirectXVideoDecoderService_GetDecoderConfigurations(ctx->decoder_service, device_guid, desc, NULL,
                                                                    &cfg_count, &cfg_list);
  if (FAILED(hr)) {
...
static int dxva2_create_decoder(AVCodecContext* s) {
  ...
  HRESULT hr = IDirectXVideoDecoderService_GetDecoderDeviceGuids(ctx->decoder_service, &guid_count, &guid_list);
  if (FAILED(hr)) {
  ...
  for (unsigned i = 0; dxva2_modes[i].guid; i++) {
    ...
    hr = IDirectXVideoDecoderService_GetDecoderRenderTargets(ctx->decoder_service, *mode->guid, &target_count,
                                                             &target_list);
    if (FAILED(hr)) {
      continue;
    }
  ...
  frames_ctx->sw_format =
      (target_format == static_cast<D3DFORMAT>(MKTAG('P', '0', '1', '0')) ? AV_PIX_FMT_P010 : AV_PIX_FMT_NV12);
  ...
  hr = IDirectXVideoDecoderService_CreateVideoDecoder(ctx->decoder_service, device_guid, &desc, &config,
                                                      frames_hwctx->surfaces, frames_hwctx->nb_surfaces,
