...
DEFINE_GUID(IID_IDirectXVideoDecoderService, 0xfc51a551, 0xd5e7, 0x11d9, 0xaf,
            0x55, 0x00, 0x05, 0x4e, 0x43, 0xff, 0x02);

DEFINE_GUID(DXVA2_ModeMPEG2_VLD, 0xee27417f, 0x5e28, 0x4e65, 0xbe, 0xea, 0x1d,
            0x26, 0xb5, 0x08, 0xad, 0xc9);
DEFINE_GUID(DXVA2_ModeMPEG2and1_VLD, 0x86695f12, 0x340e, 0x4f04, 0x9f, 0xd3,
            0x92, 0x53, 0xdd, 0x32, 0x74, 0x60);
DEFINE_GUID(DXVA2_ModeH264_E, 0x1b81be68, 0xa0c7, 0x11d3, 0xb9, 0x84, 0x00,
            0xc0, 0x4f, 0x2e, 0x73, 0xc5);
DEFINE_GUID(DXVA2_ModeH264_F, 0x1b81be69, 0xa0c7, 0x11d3, 0xb9, 0x84, 0x00,
            0xc0, 0x4f, 0x2e, 0x73, 0xc5);
DEFINE_GUID(DXVADDI_Intel_ModeH264_E, 0x604F8E68, 0x4951, 0x4C54, 0x88, 0xFE,
            0xAB, 0xD2, 0x5C, 0x15, 0xB3, 0xD6);
DEFINE_GUID(DXVA2_ModeVC1_D, 0x1b81beA3, 0xa0c7, 0x11d3, 0xb9, 0x84, 0x00, 0xc0,
            0x4f, 0x2e, 0x73, 0xc5);
DEFINE_GUID(DXVA2_ModeVC1_D2010, 0x1b81beA4, 0xa0c7, 0x11d3, 0xb9, 0x84, 0x00,
            0xc0, 0x4f, 0x2e, 0x73, 0xc5);
DEFINE_GUID(DXVA2_ModeHEVC_VLD_Main, 0x5b11d51b, 0x2f4c, 0x4452, 0xbc, 0xc3,
            0x09, 0xf2, 0xa1, 0x16, 0x0c, 0xc0);
DEFINE_GUID(DXVA2_ModeHEVC_VLD_Main10, 0x107af0e0, 0xef1a, 0x4d19, 0xab, 0xa8,
            0x67, 0xa1, 0x63, 0x07, 0x3d, 0x13);
DEFINE_GUID(DXVA2_ModeVP9_VLD_Profile0, 0x463707f8, 0xa1d0, 0x4585, 0x87, 0x6d,
            0x83, 0xaa, 0x6d, 0x60, 0xb8, 0x9e);
DEFINE_GUID(DXVA2_NoEncrypt, 0x1b81beD0, 0xa0c7, 0x11d3, 0xb9, 0x84, 0x00, 0xc0,
            0x4f, 0x2e, 0x73, 0xc5);
DEFINE_GUID(GUID_NULL, 0x00000000, 0x0000, 0x0000, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00);
...
static int dxva2_alloc(AVCodecContext *s) {
  InputStream *ist = static_cast<InputStream *>(s->opaque);
  DXVA2Context *ctx =
      static_cast<DXVA2Context *>(av_mallocz(sizeof(DXVA2Context)));
  if (!ctx) {
  ...
  int ret = av_hwdevice_ctx_create(&ctx->hw_device_ctx, AV_HWDEVICE_TYPE_DXVA2,
  ...
  AVDXVA2DeviceContext *device_hwctx =
      static_cast<AVDXVA2DeviceContext *>(device_ctx->hwctx);

  HANDLE device_handle;
  HRESULT hr = IDirect3DDeviceManager9_OpenDeviceHandle(device_hwctx->devmgr,
                                                        &device_handle);
  if (FAILED(hr)) {
  ...
  hr = IDirect3DDeviceManager9_GetVideoService(
      device_hwctx->devmgr, device_handle, IID_IDirectXVideoDecoderService,
      &pv);
  IDirect3DDeviceManager9_CloseDeviceHandle(device_hwctx->devmgr,
                                            device_handle);
  if (FAILED(hr)) {
    ERROR_LOG() << "DXVA2 failed to create IDirectXVideoDecoderService err: "
                << hr;
    dxva2_uninit(s);
    return AVERROR(EINVAL);
  }
  ...
  s->hwaccel_context =
      static_cast<struct dxva_context *>(av_mallocz(sizeof(dxva_context)));
...
static int dxva2_get_decoder_configuration(AVCodecContext *s,
  ...
  if (FAILED(hr)) {
    ERROR_LOG() << "DXVA2 unable to retrieve decoder configurations err: "
                << hr;
    return AVERROR(EINVAL);
  }
...
static int dxva2_create_decoder(AVCodecContext *s) {
  InputStream *ist = static_cast<InputStream *>(s->opaque);
  DXVA2Context *ctx = static_cast<DXVA2Context *>(ist->hwaccel_ctx);
  struct dxva_context *dxva_ctx =
      static_cast<struct dxva_context *>(s->hwaccel_context);

  const D3DFORMAT surface_format =
      (s->sw_pix_fmt == AV_PIX_FMT_YUV420P10)
  ...
  HRESULT hr = IDirectXVideoDecoderService_GetDecoderDeviceGuids(
      ctx->decoder_service, &guid_count, &guid_list);
  if (FAILED(hr)) {
  ...
  for (unsigned i = 0; dxva2_modes[i].guid; i++) {
    ...
    hr = IDirectXVideoDecoderService_GetDecoderRenderTargets(
        ctx->decoder_service, *mode->guid, &target_count, &target_list);
    if (FAILED(hr)) {
      continue;
    }
  ...
  AVDXVA2FramesContext *frames_hwctx =
      static_cast<AVDXVA2FramesContext *>(frames_ctx->hwctx);

  frames_ctx->format = AV_PIX_FMT_DXVA2_VLD;
  frames_ctx->sw_format =
      (target_format == static_cast<D3DFORMAT>(MKTAG('P', '0', '1', '0'))
           ? AV_PIX_FMT_P010
           : AV_PIX_FMT_NV12);
  ...
  if (ret < 0) {
    ERROR_LOG() << "DXVA2 failed to initialize the HW frames context err: "
                << ret;
...
int dxva2_init(AVCodecContext *s) {
  ...
  if (s->codec_id == AV_CODEC_ID_H264 &&
    ...
    ERROR_LOG() << "Unsupported H.264 profile for DXVA2 HWAccel: "
                << s->profile;
    dxva2_uninit(s);
    return AVERROR(EINVAL);
  }
