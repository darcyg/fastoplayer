...
enum AVPixelFormat get_format(AVCodecContext *s,
...
VideoState::VideoState(stream_id id, const common::uri::Url &uri,
                       const AppOptions &opt, const ComplexOptions &copt)
    : id_(id), uri_(uri), opt_(opt), copt_(copt), force_refresh_(false),
      read_pause_return_(0), ic_(NULL), realtime_(false),
      vstream_(new VideoStream), astream_(new AudioStream), viddec_(nullptr),
      auddec_(nullptr), video_frame_queue_(nullptr),
      audio_frame_queue_(nullptr), audio_clock_(0), audio_diff_cum_(0),
      audio_diff_avg_coef_(0), audio_diff_threshold_(0),
      audio_diff_avg_count_(0), audio_hw_buf_size_(0), audio_buf_(NULL),
      audio_buf1_(NULL), audio_buf_size_(0), audio_buf1_size_(0),
      audio_buf_index_(0), audio_write_buf_size_(0), audio_src_(),
#if CONFIG_AVFILTER
      audio_filter_src_(),
#endif
      audio_tgt_(), swr_ctx_(NULL), frame_timer_(0),
      frame_last_returned_time_(0), frame_last_filter_delay_(0),
      max_frame_duration_(0), step_(false),
#if CONFIG_AVFILTER
      in_video_filter_(NULL), out_video_filter_(NULL), in_audio_filter_(NULL),
      out_audio_filter_(NULL), agraph_(NULL),
#endif
      last_video_stream_(invalid_stream_index),
      last_audio_stream_(invalid_stream_index),
      vdecoder_tid_(
          THREAD_MANAGER()->CreateThread(&VideoState::VideoThread, this)),
      adecoder_tid_(
          THREAD_MANAGER()->CreateThread(&VideoState::AudioThread, this)),
      paused_(false), last_paused_(false), eof_(false), abort_request_(false),
      stats_(new Stats), handler_(nullptr),
      input_st_(static_cast<InputStream *>(calloc(1, sizeof(InputStream)))),
      seek_req_(false), seek_pos_(0), seek_rel_(0), seek_flags_(0),
      read_thread_cond_(), read_thread_mutex_() {
  ...
  if (!opt_.hwaccel_output_format.empty()) {
    ...
    if (input_st_->hwaccel_output_format == AV_PIX_FMT_NONE) {
      CRITICAL_LOG() << "Unrecognised hwaccel output format: "
...
void VideoState::SetHandler(VideoStateHandler *handler) { handler_ = handler; }
...
int VideoState::StreamComponentOpen(int stream_index) {
  ...
  if (avctx->codec_type == AVMEDIA_TYPE_VIDEO) {
    ...
    forced_codec_name =
        opt_.video_codec_name.empty() ? NULL : opt_.video_codec_name.c_str();
  } else if (avctx->codec_type == AVMEDIA_TYPE_AUDIO) {
    last_audio_stream_ = stream_index;
    forced_codec_name =
        opt_.audio_codec_name.empty() ? NULL : opt_.audio_codec_name.c_str();
  }
  ...
  if (!codec) {
    if (forced_codec_name) {
      WARNING_LOG() << "No codec could be found with name '"
                    << forced_codec_name << "'";
    } else {
  ...
  AVDictionary *opts =
      filter_codec_opts(copt_.codec_opts, avctx->codec_id, ic_, stream, codec);
  ...
  if (avctx->codec_type == AVMEDIA_TYPE_VIDEO) {
    ...
    common::Error err = handler_->HandleRequestAudio(
        this, channel_layout, nb_channels, sample_rate, &audio_tgt_,
        &audio_buff_size);
    if (err) {
      avcodec_free_context(&avctx);
      av_dict_free(&opts);
    ...
    audio_diff_threshold_ = static_cast<double>(audio_hw_buf_size_) /
                            static_cast<double>(audio_tgt_.bytes_per_sec) *
                            1000;
...
void VideoState::SeekChapter(int incr) {
  ...
  int64_t poss = av_rescale_q(ic_->chapters[chapter]->start,
                              ic_->chapters[chapter]->time_base, tb);
...
void VideoState::SeekMsec(clock64_t clock) {
  ...
  pos += clock;
  if (ic_->start_time !=
      AV_NOPTS_VALUE) { // if selected out of range move to start
...
AvSyncType VideoState::GetMasterSyncType() const { return opt_.av_sync_type; }
...
clock64_t VideoState::ComputeTargetDelay(clock64_t delay) const {
  ...
  if (GetMasterSyncType() != AV_SYNC_VIDEO_MASTER) {
    ...
    clock64_t sync_threshold = FFMAX(AV_SYNC_THRESHOLD_MIN_MSEC,
                                     FFMIN(AV_SYNC_THRESHOLD_MAX_MSEC, delay));
    if (IsValidClock(diff) && std::abs(diff) < max_frame_duration_) {
      if (diff <= -sync_threshold) {
        delay = FFMAX(0, delay + diff);
      } else if (diff >= sync_threshold &&
                 delay > AV_SYNC_FRAMEDUP_THRESHOLD_MSEC) {
        delay = delay + diff;
      } else if (diff >= sync_threshold) {
        delay = 2 * delay;
      }
...
void VideoState::Abort() { abort_request_ = true; }
...
bool VideoState::IsAborted() const { return abort_request_; }
...
stream_id VideoState::GetId() const { return id_; }

const common::uri::Url &VideoState::GetUri() const { return uri_; }
...
void VideoState::RefreshRequest() { force_refresh_ = true; }
...
bool VideoState::IsPaused() const { return paused_; }

void VideoState::ResetStats() { stats_.reset(new Stats); }
...
void VideoState::StreamCycleChannel(AVMediaType codec_type) {
  ...
  while (true) {
    ...
    AVStream *st =
        ic_->streams[p ? p->stream_index[stream_index] : stream_index];
...
common::Error VideoState::RequestVideo(int width, int height,
                                       int av_pixel_format,
  ...
  return handler_->HandleRequestVideo(this, width, height, av_pixel_format,
                                      aspect_ratio);
}
    ...
    if (IsValidClock(diff) && std::abs(diff) < AV_NOSYNC_THRESHOLD_MSEC) {
      ...
      if (audio_diff_avg_count_ < AUDIO_DIFF_AVG_NB) {
        ...
        if (fabs(avg_diff) >= audio_diff_threshold_) {
          wanted_nb_samples =
              nb_samples + static_cast<int>(diff * audio_src_.freq);
          int min_nb_samples =
              ((nb_samples * (100 - SAMPLE_CORRECTION_PERCENT_MAX) / 100));
          int max_nb_samples =
              ((nb_samples * (100 + SAMPLE_CORRECTION_PERCENT_MAX) / 100));
          wanted_nb_samples = stable_value_in_range(
...
int VideoState::AudioDecodeFrame() {
  ...
  if (af->frame->format != audio_src_.fmt ||
    ...
    if (!swr_ctx_ || swr_init(swr_ctx_) < 0) {
      ERROR_LOG() << "Cannot create sample rate converter for conversion of "
                  ...
                  << av_frame_get_channels(af->frame) << " channels to "
                  << audio_tgt_.freq << " Hz "
                  << av_get_sample_fmt_name(audio_tgt_.fmt) << " "
                  << audio_tgt_.channels << " channels!";
      swr_free(&swr_ctx_);
      return ERROR_RESULT_VALUE;
    }
  ...
  if (swr_ctx_) {
    ...
    int out_count =
        wanted_nb_samples * audio_tgt_.freq / af->frame->sample_rate + 256;
    int out_size = av_samples_get_buffer_size(NULL, audio_tgt_.channels,
    ...
    resampled_data_size =
        len2 * audio_tgt_.channels * av_get_bytes_per_sample(audio_tgt_.fmt);
  } else {
  ...
  if (IsValidClock(af->pts)) {
    const double div =
        static_cast<double>(af->frame->nb_samples) / af->frame->sample_rate;
...
retry:
  ...
  clock64_t last_duration =
      CalcDurationBetweenVideoFrames(lastvp, firstvp, max_frame_duration_);
  ...
  if (nextvp) {
    clock64_t duration =
        CalcDurationBetweenVideoFrames(firstvp, nextvp, max_frame_duration_);
...
frames::VideoFrame *VideoState::TryToGetVideoFrame() {
  ...
  stats_->fmt = (is_audio_open && is_video_open)
                    ...
                    : (is_video_open ? HAVE_VIDEO_STREAM
                                     : (is_audio_open ? HAVE_AUDIO_STREAM
                                                      : UNKNOWN_STREAM));
...
VideoState::stats_t VideoState::GetStatistic() const { return stats_; }

AVRational VideoState::GetFrameRate() const { return vstream_->GetFrameRate(); }
  ...
  while (len > 0) {
    if (audio_buf_index_ >= audio_buf_size_) {
      ...
      if (audio_size < 0) {
        ...
        audio_buf_ = NULL;
        audio_buf_size_ = AUDIO_MIN_BUFFER_SIZE / audio_tgt_.frame_size *
                          audio_tgt_.frame_size;
      } else {
    ...
    if (audio_buf_ && audio_volume == 100) {
      ...
      if (audio_buf_) {
        if (handler_) {
          handler_->HanleAudioMix(stream, audio_buf_ + audio_buf_index_, len1,
                                  audio_volume);
        }
      }
    }
...
int VideoState::QueuePicture(AVFrame *src_frame, clock64_t pts,
                             clock64_t duration, int64_t pos) {
  ...
  if (vp->width != src_frame->width || vp->height != src_frame->height ||
    ...
    if (handler_) {
      handler_->HandleFrameResize(this, vp->width, vp->height, vp->format,
                                  vp->sar);
    }
  }
  ...
  if (got_picture) {
    ...
    if (opt_.framedrop == FRAME_DROP_AUTO ||
      ...
      if (IsValidPts(frame->pts)) {
        ...
        PacketQueue *video_packet_queue = vstream_->GetQueue();
        if (IsValidClock(diff) && std::abs(diff) < AV_NOSYNC_THRESHOLD_MSEC &&
            diff - frame_last_filter_delay_ < 0 &&
...
int VideoState::ReadRoutine() {
  ...
  if (!ic) {
    ...
    if (handler_) {
      handler_->HandleQuitStream(this, av_errno,
                                 common::make_error_from_errno(err));
    }
    return ERROR_RESULT_VALUE;
  }
  ...
  if (!av_dict_get(copt_.format_opts, "scan_all_pmts", NULL,
                   AV_DICT_MATCH_CASE)) {
    av_dict_set(&copt_.format_opts, "scan_all_pmts", "1",
                AV_DICT_DONT_OVERWRITE);
    scan_all_pmts_set = true;
  }

  int open_result = avformat_open_input(
      &ic, in_filename, NULL, &copt_.format_opts); // autodetect format
  if (open_result < 0) {
  ...
  max_frame_duration_ =
      (ic->iformat->flags & AVFMT_TS_DISCONT) ? 10000 : 3600000;

  if (opt_.seek_by_bytes == SEEK_AUTO) {
    bool seek = (ic->iformat->flags & AVFMT_TS_DISCONT) &&
  ...
  for (int i = 0; i < static_cast<int>(ic->nb_streams); i++) {
    ...
    if (type >= 0 && !opt_.wanted_stream_spec[type].empty() &&
        st_index[type] == -1) {
  ...
  for (int i = 0; i < static_cast<int>(AVMEDIA_TYPE_NB); i++) {
    if (!opt_.wanted_stream_spec[i].empty() && st_index[i] == -1) {
      ERROR_LOG() << "Stream specifier " << opt_.wanted_stream_spec[i]
                  << " does not match any "
  ...
  if (ic_->bit_rate) {
    ...
    if (!band.InRange(byte_per_sec)) {
      WARNING_LOG() << "Stream bitrate is: " << byte_per_sec
                    << " not in our calculation range(" << band.min << "/"
                    << band.max << ").";
    }
  }
  ...
  while (!IsAborted()) {
    ...
    if (seek_req_) {
      ...
      int64_t seek_min =
          seek_rel_ > 0 ? seek_target - seek_rel_ + 2 : INT64_MIN;
      int64_t seek_max =
          seek_rel_ < 0 ? seek_target - seek_rel_ - 2 : INT64_MAX;
      // FIXME the +-2 is due to rounding being not done in the correct
      // direction in generation
      //      of the seek_pos/seek_rel variables

      int ret = avformat_seek_file(ic, -1, seek_min, seek_target, seek_max,
                                   seek_flags_);
      if (ret < 0) {
        ERROR_LOG() << "Seeking " << id_
                    << "failed error: " << ffmpeg_errno_to_string(ret);
      } else {
        if (video_stream->IsOpened()) {
    ...
    if (opt_.infinite_buffer < 1 &&
        (video_packet_queue->GetSize() + audio_packet_queue->GetSize() >
             MAX_QUEUE_SIZE ||
    ...
    if (!paused_ && eof_) {
      ...
      if (is_audio_dec_ready) {
        is_audio_not_finished_but_empty =
            !auddec_->IsFinished() && audio_frame_queue_->IsEmpty();
      }
      bool is_video_dec_ready = viddec_ && video_frame_queue_;
      bool is_video_not_finished_but_empty = false;
      if (is_video_dec_ready) {
        is_video_not_finished_but_empty =
            !viddec_->IsFinished() && video_frame_queue_->IsEmpty();
      }
      if ((!is_audio_dec_ready ||
           (is_audio_dec_ready && is_audio_not_finished_but_empty)) &&
          (!is_video_dec_ready ||
           (is_video_dec_ready && is_video_not_finished_but_empty)) &&
          opt_.auto_exit) {
    ...
    if (ret < 0) {
      WARNING_LOG() << "Read input stream error: "
                    << ffmpeg_errno_to_string(ret);
...
int VideoState::AudioThread() {
  ...
  do {
    ...
    if (got_frame) {
      ...
      int reconfigure =
          cmp_audio_fmts(audio_filter_src_.fmt, audio_filter_src_.channels,
      ...
      if (reconfigure) {
        ...
        av_get_channel_layout_string(buf1, SIZEOFMASS(buf1), -1,
                                     audio_filter_src_.channel_layout);
        av_get_channel_layout_string(buf2, SIZEOFMASS(buf2), -1,
                                     dec_channel_layout);
        const std::string mess = common::MemSPrintf(
            ...
            av_get_sample_fmt_name(audio_filter_src_.fmt), buf1, 0,
      ...
      while ((ret = av_buffersink_get_frame_flags(out_audio_filter_, frame,
                                                  0)) >= 0) {
        ...
        af->pts = IsValidPts(frame->pts) ? frame->pts * q2d_diff(tb)
                                         : invalid_clock();
...
int VideoState::VideoThread() {
  ...
  while (true) {
    ...
    if (input_st_->hwaccel_retrieve_data &&
        frame->format == input_st_->hwaccel_pix_fmt) {
      int err = input_st_->hwaccel_retrieve_data(viddec_->GetAvCtx(), frame);
      if (err < 0) {
        continue;
      }
    }
    input_st_->hwaccel_retrieved_pix_fmt =
        static_cast<AVPixelFormat>(frame->format);

#if CONFIG_AVFILTER
    if (last_w != frame->width || last_h != frame->height ||
        last_format != frame->format) { // -vf option
      const std::string mess = common::MemSPrintf(
          "Video frame changed from size:%dx%d format:%s serial:%d to "
          "size:%dx%d format:%s "
          "serial:%d",
          last_w, last_h, static_cast<const char *>(av_x_if_null(
                              av_get_pix_fmt_name(last_format), "none")),
    ...
    while (ret >= 0) {
      ...
      clock64_t duration =
          (frame_rate.num && frame_rate.den ? q2d_diff(fr) : 0);
      clock64_t pts =
          IsValidPts(frame->pts) ? frame->pts * q2d_diff(tb) : invalid_clock();
...
int VideoState::ConfigureVideoFilters(AVFilterGraph *graph,
  ...
  static const enum AVPixelFormat pix_fmts[] = {
      AV_PIX_FMT_YUV420P, AV_PIX_FMT_BGRA, AV_PIX_FMT_NONE};
  ...
  snprintf(buffersrc_args, sizeof(buffersrc_args),
           ...
           frame->width, frame->height, frame->format, tb.num, tb.den,
  ...
  ret = av_opt_set_int_list(filt_out, "pix_fmts", pix_fmts, AV_PIX_FMT_NONE,
...
int VideoState::ConfigureAudioFilters(const std::string &afilters,
  ...
  ret = av_opt_set_int(filt_asink, "all_channel_counts", 1,
                       AV_OPT_SEARCH_CHILDREN);
  if (ret < 0) {
    avfilter_graph_free(&agraph_);
    return ret;
  ...
  if (force_output_format) {
    ...
    ret = av_opt_set_int(filt_asink, "all_channel_counts", 0,
                         AV_OPT_SEARCH_CHILDREN);
    if (ret < 0) {
      avfilter_graph_free(&agraph_);
      return ret;
    }
    ret = av_opt_set_int_list(filt_asink, "channel_layouts", channel_layouts,
                              -1, AV_OPT_SEARCH_CHILDREN);
    if (ret < 0) {
      avfilter_graph_free(&agraph_);
      return ret;
    }
    ret = av_opt_set_int_list(filt_asink, "channel_counts", channels, -1,
                              AV_OPT_SEARCH_CHILDREN);
    if (ret < 0) {
      avfilter_graph_free(&agraph_);
      return ret;
    }
    ret = av_opt_set_int_list(filt_asink, "sample_rates", sample_rates, -1,
                              AV_OPT_SEARCH_CHILDREN);
    if (ret < 0) {
      avfilter_graph_free(&agraph_);
      return ret;
