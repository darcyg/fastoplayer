...
IListBox::IListBox()
    : base_class(), row_height_(0), last_drawed_row_pos_(0),
      selection_(NO_SELECT), selection_color_(),
      preselected_row_(draw::invalid_row_position), mouse_clicked_row_cb_(),
      mouse_released_row_cb_() {}

IListBox::IListBox(const SDL_Color &back_ground_color)
    : base_class(back_ground_color), row_height_(0), last_drawed_row_pos_(0),
      selection_(NO_SELECT), selection_color_(),
      preselected_row_(draw::invalid_row_position), mouse_clicked_row_cb_(),
      mouse_released_row_cb_() {}
...
void IListBox::SetSelection(Selection sel) { selection_ = sel; }

int IListBox::GetSelection() const { return selection_; }

void IListBox::SetSelectionColor(const SDL_Color &sel) {
  selection_color_ = sel;
}

SDL_Color IListBox::GetSelectionColor() const { return selection_color_; }

void IListBox::SetRowHeight(int row_height) { row_height_ = row_height; }

int IListBox::GetRowHeight() const { return row_height_; }
...
void IListBox::Draw(SDL_Renderer *render) {
  ...
  int drawed = 0;
  for (size_t i = last_drawed_row_pos_;
       i < GetRowCount() && drawed < max_line_count; ++i) {
    SDL_Rect cell_rect = {draw_area.x, draw_area.y + row_height_ * drawed,
                          draw_area.w, row_height_};
...
ListBox::ListBox(const SDL_Color &back_ground_color)
    : base_class(back_ground_color), lines_() {}

ListBox::~ListBox() {}

void ListBox::SetLines(const lines_t &lines) { lines_ = lines; }

ListBox::lines_t ListBox::GetLines() const { return lines_; }

size_t ListBox::GetRowCount() const { return lines_.size(); }

void ListBox::DrawRow(SDL_Renderer *render, size_t pos, bool is_active_row,
