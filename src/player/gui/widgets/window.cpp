...
Window::Window(const SDL_Color &back_ground_color)
    : rect_(draw::empty_rect), back_ground_color_(back_ground_color),
      border_color_(draw::black_color), visible_(false), transparent_(false),
      bordered_(false), focus_(false), enabled_(true), min_size_(),
      mouse_clicked_cb_(), mouse_released_cb_(), focus_changed_cb_(),
      enable_chaned_cb_() {
  Init();
}
...
void Window::SetRect(const SDL_Rect &rect) { rect_ = rect; }
...
SDL_Color Window::GetBackGroundColor() const { return back_ground_color_; }

void Window::SetBackGroundColor(const SDL_Color &color) {
  back_ground_color_ = color;
}

SDL_Color Window::GetBorderColor() const { return border_color_; }

void Window::SetBorderColor(const SDL_Color &color) { border_color_ = color; }

void Window::SetMinimalSize(const common::draw::Size &ms) { min_size_ = ms; }

common::draw::Size Window::GetMinimalSize() const { return min_size_; }

void Window::SetTransparent(bool t) { transparent_ = t; }

bool Window::IsTransparent() const { return transparent_; }

bool Window::IsBordered() const { return bordered_; }

void Window::SetBordered(bool b) { bordered_ = b; }
...
bool Window::IsEnabled() const { return enabled_; }
...
bool Window::IsFocused() const { return focus_; }

void Window::Show() { SetVisible(true); }

void Window::Hide() { SetVisible(false); }

void Window::ToggleVisible() { SetVisible(!IsVisible()); }
...
void Window::HandleEvent(event_t *event) {
  if (event->GetEventType() == gui::events::WindowResizeEvent::EventType) {
    ...
    HandleWindowCloseEvent(window_close);
             ...
             gui::events::MouseStateChangeEvent::EventType) {
    gui::events::MouseStateChangeEvent *mouse_state =
        static_cast<gui::events::MouseStateChangeEvent *>(event);
    ...
    HandleMousePressEvent(mouse_press);
             ...
             gui::events::MouseReleaseEvent::EventType) {
    gui::events::MouseReleaseEvent *mouse_release =
        static_cast<gui::events::MouseReleaseEvent *>(event);
...
void Window::HandleMouseStateChangeEvent(
    gui::events::MouseStateChangeEvent *event) {
  if (!IsCanDraw()) {
    return;
  }
